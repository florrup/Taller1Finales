...
void procesar() {
	...
	while ( hayAtaque || hayMovimiento ) {
		...
		for (; itE != itEFIN; itE++) {
			...
			// Recorro una vez para contar cantidad de personajes en rango
			for (; itP != itPFIN; itP++) {
			...
			// Vuelvo a recorrer los personajes para atacarlos y distribuir el daño

			itP = listadoPersonajes.begin();
			itPFIN = listadoPersonajes.end();
			...
			for (; itP != itPFIN; itP++) {
				...
				if ( (*itP)->vivo ) {
					...
					if ( (*itP)->x >= x0 && (*itP)->x <= xf && (*itP)->y >= y0 && (*itP)->y <=yf ) {
						...
						// Se divide el ataque por la cantidad de personajes que hay en el rango en ese momento
						(*itP)->energia -= (*itE)->ataque / cantPJenRango;
